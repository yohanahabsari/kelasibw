<%@ WebHandler Language="VB" Class="kelasDashHandler" %>
...
Public Class kelasDashHandler : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        Try
            ukuran = Val(context.Request("size").ToString)

        Catch ex As Exception
            ukuran = 100
        End Try
        namafile = context.Request("namafile")
        If ukuran > 0 Then
            imageByte = bikinDash(context.Request("kata"), namafile, ukuran)
        Else
            imageByte = bikinDash(context.Request("kata"), namafile)
    ...
    Function loadImage(namafile As String) As Byte()
        ...
        Return imageByte
    End Function
    Private Function bikinDash(kata As String, namafile As String, Optional size As Integer = 100) As Byte()
        ...
        x = CInt(Math.Truncate(size * 5 / 100))
        lebar = CInt(Math.Truncate(size * 90 / 100))
        canvas.FillRectangle(New SolidBrush(warna), 0, 0, size, size)
        canvas.DrawRectangle(New Pen(Color.White), x, x, lebar, lebar)
        If namafile = "" Then
            Dim kata1, kata2 As String
            Dim letakspasi As Integer
            kata = kata & " "
            letakspasi = InStr(kata, " ")
            If letakspasi < 1 Then letakspasi = 1
            kata1 = Mid(kata, 1, letakspasi)
            kata2 = Mid(kata, letakspasi + 1, kata.Length)
            canvas.DrawString(kata1, New Font("Verdana", 12,
                     FontStyle.Bold), New SolidBrush(Color.Black), ((5)),
            (size / 4))
            canvas.DrawString(kata2, New Font("Verdana", 12,
                FontStyle.Bold), New SolidBrush(Color.Black), ((5)),
       (size / 3 * 2))
        Else
            namafile = HttpContext.Current.Server.MapPath("~/") & namafile
            img = System.Drawing.Image.FromFile(namafile)
            img = ScaleImageWidth(img, size)
            size2.Width = img.Width
            size2.Height = img.Height
            loBMP = New Bitmap(img)
            canvas.DrawImage(loBMP, New Rectangle(New Point(0, 0), size2))
            'canvas.DrawString(kata, New Font("Verdana", 18,
            '            FontStyle.Bold), New SolidBrush(Color.Black), ((5)),
            '(300 / 3 * 2))
        End If
